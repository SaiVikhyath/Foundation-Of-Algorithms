...
"""
Description:
Given a directed graph, G = (V, E) and a source vertex (S),
...
class Dijkstra:
        ...
        for i in range(self.numberOfVertices):
            for j in range(self.numberOfVertices):
                if i == j:
                    ...
                    self.graph[i][j] = float(input("Enter the distance between vertex " + str(i) + " and vertex " + str(j) + ": "))
    ...
    def dijkstra(self) -> None:
        ...
        print("Vertices selected for shortest path:", shortestPath)
